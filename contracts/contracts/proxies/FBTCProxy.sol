...
contract FBTCProxy is AccessControlUpgradeable {
    ...
    function mintLockedFbtcRequest(
        ...
        bytes memory data = abi.encodeWithSelector(
            ILockedFBTC.mintLockedFbtcRequest.selector,
            _amount
        );
    ...
    function redeemFbtcRequest(
        ...
        bytes memory data = abi.encodeWithSelector(
            ILockedFBTC.redeemFbtcRequest.selector,
            _amount,
            _depositTxid,
            _outputIndex
        );
     ...
     */
    function confirmRedeemFbtc(
        uint256 _amount
    ) external onlyRole(OPERATOR_ROLE) {
        bytes memory data = abi.encodeWithSelector(
            ILockedFBTC.confirmRedeemFbtc.selector,
            _amount
        );
        IVault(vault).execute(lockedFBTC, data, 0);
    }
     ...
     */
    function burn(uint256 _amount) external onlyRole(OPERATOR_ROLE) {
        bytes memory data = abi.encodeWithSelector(
            ILockedFBTC.burn.selector,
            _amount
        );
        IVault(vault).execute(lockedFBTC, data, 0);
    }
