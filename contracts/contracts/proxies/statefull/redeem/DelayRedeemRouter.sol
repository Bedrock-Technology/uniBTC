    ...
    ReentrancyGuardUpgradeable
{
    ...
    // the duration time in 30 days (60 * 60 * 24 * 30 = 2,592,000)
    uint256 public constant MAX_REDEEM_DELAY_DURATION_TIME = 2592000;
    // the duration time in 1 day (60 * 60 * 24 = 86,400)
    uint256 public constant DAY_DELAY_DURATION_TIME = 86400;
    // the max Cap in 1 day
    uint256 public constant DAY_MAX_ALLOWED_CAP = 1000e8;
    /**
     * @notice Delay enforced by this contract for completing any delayedRedeem, Measured in timestamp, and adjustable by this contract's owner,
     * up to a maximum of `MAX_REDEEM_DELAY_DURATION_TIME`. Minimum value is 0 (i.e. no delay enforced).
     */
    ...
    //uniBTC erc20 address
    address public uniBTC;

    //uniBTC vault address
    ...
    // struct used to store a single users delayedRedeem data
    struct UserDelayedRedeems {
        uint256 delayedRedeemsCompleted;
        DelayedRedeem[] delayedRedeems;
    }

    // define a structure for temporary storage of ERC20 tokens and corresponding cumulative amounts
    struct DebtTokenAmount {
        address token;
        uint256 amount;
    }

    // track the debt token info
    struct TokenDebtInfo {
        uint256 totalAmount;
        uint256 claimedAmount;
    }

    //Mapping: user => struct storing all delayedRedeem info. Marked as internal with an external getter function named `userRedeems`
    mapping(address => UserDelayedRedeems) internal _userRedeems;

    //track different token debt
    mapping(address => TokenDebtInfo) public tokenDebts;

    // only whitelist address can redeem using unibtc
    mapping(address => bool) private whitelist;

    // only wrapBtcList token address can redeem using unibtc
    mapping(address => bool) private wrapBtcList;

    // default value for whitelistEnabled is true
    bool public whitelistEnabled;

    // one day can redeem amount
    uint256 public dayCap;

    //track total Cap
    uint256 private _totalCap;

    //track last updated day
    ...
    function getClaimableUserDelayedRedeems(
        ...
        for (uint256 i = 0; i < userDelayedRedeemsLength; i++) {
            ...
            if (block.timestamp < delayedRedeem.timestampCreated + redeemDelayTimestamp) {
