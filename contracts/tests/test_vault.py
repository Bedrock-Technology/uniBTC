...
def test_mint_native(fn_isolation, contracts, owner, alice):
    uni_btc, vault, fbtc = contracts[0], contracts[6], contracts[7]

    # ---Revert Path Testing---

    # Mint reverts because it is only available in the Bitcoin ecosystem
    assert not vault.isNativeBTC()
    with brownie .reverts("SYS010"):
        vault.mint({'from': alice, 'value': 0})
...
def test_redeem_native(fn_isolation, contracts, owner, alice):
    ...
    # ---Revert Path Testing---
    # Scenario 1: Redeem reverts because it is only available in the Bitcoin ecosystem
    assert not vault.isNativeBTC()
    with brownie .reverts("SYS010"):
...
def test_setCap(fn_isolation, contracts, owner, alice, zero_address):
    wbtc18, wbtc, xbtc, vault = contracts[10], contracts[5], contracts[8], contracts[6]
    ...
    #  Scenario 3: Not allowed to set cap for native BTC
    assert not vault.isNativeBTC()
    with brownie .reverts("SYS010"):
        vault.setCap(vault.NATIVE_BTC(), cap, {'from': owner})

    # Scenario 4: The decimals of the given token must be equal to 8 or 18
...
def test_adminWithdraw_native(fn_isolation, contracts, owner, alice, bob, zero_address):
    ...
    # Scenario 1: Revert because not available in non-Bitcoin ecosystem
    assert not vault.isNativeBTC()
    with brownie.reverts("SYS010"):
