...
def test_claimFromRedeemRouter(deps):
    ...
    transparent_delay_redeem_router.initialize(
        owner, uniBTC_proxy, vault_proxy, seven_day_time_duration, True, {"from": owner}
    )
    ...
    transparent_delay_redeem_router.addToWrapBtcList(
        [fbtc_contract, wbtc_contract, native_token], {"from": owner}
    )
    transparent_delay_redeem_router.removeFromWrapBtcList(
        [fbtc_contract, wbtc_contract, native_token], {"from": owner}
    )
    transparent_delay_redeem_router.addToWrapBtcList(
        [fbtc_contract, wbtc_contract], {"from": owner}
    )
    transparent_delay_redeem_router.addToWrapBtcList(
        [native_token], [native_claim_uni * 9 / 10], {"from": owner}
    )
    ...
    fbtc_day_cap = 100 * 10**8
    wbtc_day_cap = 80 * 10**8
    native_day_cap = 5 * 10**8
    tx = transparent_delay_redeem_router.setDayCap(
        [fbtc_contract, wbtc_contract, native_token],
        [fbtc_day_cap, wbtc_day_cap, native_day_cap],
        {"from": owner},
    )
    assert "DayCapSet" in tx.events
    assert tx.events["DayCapSet"][0]["token"] == fbtc_contract
    assert tx.events["DayCapSet"][0]["previousValue"] == 0
    assert tx.events["DayCapSet"][0]["newValue"] == fbtc_day_cap
    assert tx.events["DayCapSet"][1]["token"] == wbtc_contract
    assert tx.events["DayCapSet"][1]["previousValue"] == 0
    assert tx.events["DayCapSet"][1]["newValue"] == wbtc_day_cap
    assert tx.events["DayCapSet"][2]["token"] == native_token
    assert tx.events["DayCapSet"][2]["previousValue"] == 0
    assert tx.events["DayCapSet"][2]["newValue"] == native_day_cap

    # effect the second day
    with brownie.reverts("USR010"):
        tx = transparent_delay_redeem_router.createDelayedRedeem(
            fbtc_contract, fbtc_claim_uni, {"from": user}
        )
    one_day_travel = 86400
    chain.sleep(one_day_travel)
    chain.mine()
    assert (
        transparent_delay_redeem_router.getAvailableCap(fbtc_contract)
        == fbtc_day_cap
    )
    tx = transparent_delay_redeem_router.createDelayedRedeem(
        fbtc_contract, fbtc_claim_uni, {"from": user}
    ...
    print(
        "getTokenDebt fbtc_contract",
        transparent_delay_redeem_router.getTokenDebt(fbtc_contract),
    )
    assert (
        transparent_delay_redeem_router.getTokenDebt(fbtc_contract)[0] == fbtc_claim_uni
    )
    assert transparent_delay_redeem_router.canClaimDelayedRedeem(user, 0) == False
    assert (
        transparent_delay_redeem_router.getAvailableCap(fbtc_contract)
        == fbtc_day_cap - fbtc_claim_uni
    )
    assert (
        transparent_delay_redeem_router.getAvailableCap(wbtc_contract)
        == wbtc_day_cap
    )
    ...
    assert (
        transparent_delay_redeem_router.getTokenDebt(wbtc_contract)[0] == wbtc_claim_uni
    )
    assert transparent_delay_redeem_router.canClaimDelayedRedeem(user, 1) == False
    assert (
        transparent_delay_redeem_router.getAvailableCap(wbtc_contract)
        == 8 * wbtc_day_cap - wbtc_claim_uni
    )
    ...
    transparent_delay_redeem_router.addToWrapBtcList(
        [native_token], [native_claim_uni], {"from": owner}
    )
    tx = transparent_delay_redeem_router.createDelayedRedeem(
        native_token, native_claim_uni, {"from": user}
    )
    ...
    assert (
        transparent_delay_redeem_router.getTokenDebt(fbtc_contract)[1] == fbtc_claim_uni
    )
    assert (
        transparent_delay_redeem_router.getTokenDebt(wbtc_contract)[1] == wbtc_claim_uni
    )
    assert (
        transparent_delay_redeem_router.getTokenDebt(native_token)[1] == native_claim_uni
    )
    ...
    print(
        "wbtc token debt info",
        transparent_delay_redeem_router.getTokenDebt(wbtc_contract),
    )
    print(
        "fbtc token debt info",
        transparent_delay_redeem_router.getTokenDebt(fbtc_contract),
    )
    print(
        "native token debt info",
        transparent_delay_redeem_router.getTokenDebt(native_token),
    )
    print(
        "wbtc supply Info",
        transparent_delay_redeem_router.getWrapBtcSupply(wbtc_contract),
    )
    print(
        "fbtc supply Info",
        transparent_delay_redeem_router.getWrapBtcSupply(fbtc_contract),
    )
    print(
        "native supply Info",
        transparent_delay_redeem_router.getWrapBtcSupply(native_token),
    )
    print(
        "undefine token supply Info",
        transparent_delay_redeem_router.getWrapBtcSupply(vault_contract),
    )
